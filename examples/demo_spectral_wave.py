...
def run_demo(elem_order:int, nelem_x:int, nelem_y:int,dt: float,
             bump_x:float = 0.3, bump_y:float = 0.6):
    ...
    char_wavenum = 10
    init_cond_U = lambda x,y: np.exp((-0.5*char_wavenum**2)*((x-bump_x)**2 + (y-bump_y)**2))
    init_cond_UDOT = lambda x,y: np.zeros(np.broadcast_shapes(x.shape,y.shape))
    ...
    for i in range(nelem_x):
        for j in range(nelem_y):
            ...
            U[global_indices[i,j]] = init_cond_U(coords[i,j,:,:,0],coords[i,j,:,:,1])
            UDOT[global_indices[i,j]] = init_cond_UDOT(coords[i,j,:,:,0],coords[i,j,:,:,1])
    U[-1] = 0
    UDOT[-1] = 0
...
Number of elements along the y-axis
    ...
    parser.add_argument("-s","--step","--dt",help="""
Time step size
    """,action="store",default=1e-3,type=float,metavar="DT")
    parser.add_argument("-t","--tmax",help="""
Simulation end time
    """,action="store",default=5,type=float,metavar="TMAX")
...
if __name__ == "__main__":
    ...
    print(f"Demo-ing spectral elements: order {elem_order} on a "+
    ...
    for i,t,coords,u in run_demo(elem_order,nelemx,nelemy,dt):
        ...
        print(f"step {i:6d} (t = {t:8.4f}): max |u| = {np.max(np.abs(u)):5.2f}",end="\r")
