...
def test_deriv(polynomials):
    ...
    for p in polynomials:
        ...
        for x in np.linspace(-20,20,300):
            cdiff = (_poly_util.polyeval(p,x+h) - _poly_util.polyeval(p,x-h)) / twoh
...
def test_polyprod(polynomials):
    for p in polynomials:
        for q in polynomials:
            for x in np.linspace(-20,20,300):
                ref = pytest.approx(_poly_util.polyeval(p,x) * _poly_util.polyeval(q,x))
                assert _poly_util.polyeval(_poly_util._polyprod(p,q,clear_zeros=True),x) == ref
                assert _poly_util.polyeval(_poly_util._polyprod(p,q,clear_zeros=False),x) == ref
...
def test_integ(polynomials):
    ...
    for p in polynomials:
        ...
        for x in np.linspace(-20,20,300):
            assert _poly_util.polyeval(p,x) == pytest.approx(_poly_util.polyeval(dintp,x)),\
...
def test_polydot(polynomials):
    ...
    for p in polynomials:
        for q in polynomials:
            assert _poly_util.polydot(p,q) == pytest.approx(_poly_util._polyinteg(_poly_util._polyprod(p,q)))
